...
add "▶ Normas activas (~/.codex/AGENTS.md): secretos = nunca leer/imprimir .env/credenciales (guard PreToolUse activo); repos denegados (política local de deny) = deny de exfiltración/push; SOP + quien construye no aprueba (otro agente audita); rastro narrativo (session log + CHANGELOG); un agente vivo a la vez (RAM 4GB)."
