...
describe("isClientPath", () => {
  test("rechaza paths que resuelven bajo denied-alpha o denied-beta (segmento exacto)", () => {
    expect(isClientPath("/home/dev/repos/denied-alpha")).toBe(true);
    expect(isClientPath("/home/dev/repos/denied-alpha/sub/dir")).toBe(true);
    expect(isClientPath("/home/dev/work/denied-beta")).toBe(true);
    expect(isClientPath("/home/dev/work/DENIED-BETA/src")).toBe(true); // case-insensitive
  });
  test("permite paths propios, incl. los que solo CONTIENEN el nombre como substring de otra palabra", () => {
    expect(isClientPath("/home/dev/eBrain")).toBe(false);
    expect(isClientPath("/home/dev/repos/denied-alpha-notes")).toBe(false); // no es un segmento exacto
    expect(isClientPath("/home/dev/second-brain")).toBe(false);
  });
});
...
test("newSession: cwd bajo repo de cliente → deny-client, nunca llega a crear la sesión tmux", async () => {
  const r = await newSession("test", "cliente-test", { cwd: "/home/dev/repos/denied-alpha/sub" });
  expect(r.ok).toBe(false);
