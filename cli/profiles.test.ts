...
describe("execution profiles -- ADR-005", () => {
  ...
  test("validates an exact, secret-free schema", () => {
    ...
    expect(() => parseProfileStore({ ...store, token: "forbidden" })).toThrow("schema_version=1 requerido");
    expect(() => parseProfileStore({ ...store, profiles: [{ ...store.profiles[0], api_key: "forbidden" }] })).toThrow("campos desconocidos");
    expect(() => parseProfileStore({ ...store, profiles: [{ ...store.profiles[0], capabilities: { coding: ["bad model with spaces"] } }] })).toThrow("capability/modelos invalidos");
  });
  ...
  test("requires catalog provenance before a user profile can select a new model", () => {
    ...
    expect(() => addExecutionProfile(store, profile)).toThrow("no existe en catalogo");
    ...
    expect(() => addExecutionProfile(next, profile)).toThrow("perfil ya existe");
  });
  ...
  test("rejects a malformed existing store instead of silently using it", async () => {
    ...
    await expect(readProfileStore(path)).rejects.toThrow("store de perfiles invalido");
  });
});
