...
- [x] D.5.3 **GATE criterio 4:** los tests corren en la suite CI (`bun test ./cli/isolation.test.ts` = 6/6). **Nivel de enforcement (corregido en auditoría Opus 2026-07-14 + cierre Codex 2026-07-15):** el invariante está cubierto por (a) el CI test del módulo puro, (b) federación default-deny en discovery, (c) `doctor.sh` daemon-aware vía MCP, y (d) preflight de boot D.5.4. **(Corrección Fable 2026-07-15, finding F-F2):** el probe `ebrain q "brisas dekko cliente"` que se citó como "verificación viva" era evidencia vacua y queda descartado.
- [x] D.5.4 **(maker/Codex, cierre 2026-07-15)** Cableado `assertNoClientSources()` al preflight de `scripts/ebrain-brain`: `cli/daemon-preflight.ts` lista los sources configurados con el engine local antes de `serve --http`, valida id/name/local_path contra el denylist y hard-falla el boot si aparece un source de cliente. **Verify:** `cli/daemon-preflight.test.ts` cubre source `brisas`/`dekko`; `ebrain daemon restart` levantó healthy después del preflight; `ebrain doctor --json` reporta `sources:isolation ok` vía daemon MCP.
...
- [x] D.7.1 **GATE FD `[AUDIT_PASS]` (Opus, checker, 2026-07-14):** los 4 criterios GO satisfechos —
  ...
  (4) aislamiento cliente con test = PASS a nivel test+vivo (D.5) **con caveat en ese momento**: la enforcement en boot del host
  quedó registrada como D.5.4 y fue cerrada después por Codex (2026-07-15). Suites verdes: `bun test ./cli/` 135/0,
