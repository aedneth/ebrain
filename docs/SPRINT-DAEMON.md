...
- [x] D.7.2 **Fable 5** auditó la fase (segundo checker independiente, maker≠checker) — **`[FABLE_AUDIT_PASS]` 2026-07-15**. Confirmó D.6 con evidencia propia (8 clientes MCP concurrentes = 94 tools en 0.163s, serve=1, 401 sin/con-bearer-inválido) + los 4 gates + suites. Reporte: `docs/AUDIT-FABLE-FASE-D.md`. Aportó 2 findings nuevos (F-F1, F-F2) + subió F-D1 a media + detalle en F-D2.
...
### Findings de auditoría (Opus 2026-07-14 · revalidados/ampliados por Fable 5 2026-07-15)
...
- **F-D1 (media — Fable subió la severidad):** `assertNoClientSources()` NO está cableada al boot del host; solo la ejerce el CI test. Los docs (D.5.3 + docstring de `cli/isolation.ts`) lo afirmaban como "enforced en runtime" → corregidos. **Fable observó que 2 de las 3 capas compensatorias están dormidas bajo el daemon:** el fail-check de `doctor.sh` difiere SIEMPRE que haya un `serve` vivo (= siempre, ahora), y `ebrain-q` está roto (F-F1). Queda federación default-deny en discovery + el CI test como enforcement real; **D.5.4 pasa a ser la única enforcement viva futura del plano-source → prioridad ALTA.**
- **F-D2 (baja + un item real):** claude/gemini/opencode/cursor guardan el bearer en reposo en sus configs; solo **codex** usa indirección env-var (`--bearer-token-env-var`, lo correcto). **Fable encontró lo que Opus no vio (confirmado):** `~/.gemini/settings.json` está en **664 world-readable** (las demás configs = 600) con el bearer literal → **fix inmediato `chmod 600 ~/.gemini/settings.json`**. Además el token transita por argv (`/proc/cmdline`) durante `mcp add` en claude/gemini/opencode. Hardening antes del release público.
- **F-F1 (media — NUEVO, Fable · confirmado por Opus) — superficie CLI/write-back rota bajo el daemon:** las ops CLI del engine (`ebrain q`, `sync`, `dream-cycle`, `sessions-federate`) abren PGLite directo y **contienden con el lock del daemon** → `ebrain q` cuelga hasta timeout (Opus reprodujo: `ebrain q "korvex"` = **rc=124, 40s**) o traga el error con rc=0; `remember` degrada y "el próximo sweep" NUNCA llega con el serve vivo. **Los learnings escritos por CLI no se embeben ni son buscables con el daemon UP** (los que escribe un agente por el tool MCP `remember` sí, van al daemon). Root cause: NO hay `remote_mcp` en `~/.gbrain/config.json` (confirmado) → el rewire thin-client de las ops CLI no se hizo. **NO bloquea los gates** (inject-context usa MCP, no CLI), pero es **cierre obligatorio (maker):** (a) rutear ops CLI por el daemon (thin-client `remote_mcp`); (b) `dream-cycle`/`doctor` daemon-aware; (c) `ebrain-q` debe fallar ruidoso (no rc=0 con vacío).
