...
- Embeddings: el default out-of-the-box (`openrouter:openai/text-embedding-3-small`, 1536 dims) rutea por OpenRouter y reusa la misma key/cap que el routing — ahora sí toca este presupuesto (el costo de embeddings es una fracción chica del gasto de chat ruteado). Alternativa local $0: Ollama `nomic-embed-text` (768 dims), que queda fuera de este presupuesto. Ver `docs/memory/embedding-providers.md`.
