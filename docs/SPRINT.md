...
- [ ] 2.4 MCP: `claude mcp add ebrain -- gbrain serve` (stdio local) **+ registrar MCP de graphify** (dos toolsets). Probar tools tipados desde una sesión Claude Code; validar cross-source vía `all_sources:true`.
- [ ] 2.5 `.gbrain-source` pin + bloque `## GBrain Search Guidance` **+ `## Code Structure Guidance`** (ruteo a graphify) en CLAUDE.md de repos PROPIOS (no cliente). Commit local, SIN push korvex-*.
...
- [ ] 2.6c **[NUEVO — Eduardo] Pipeline `brain-init`** (agent-agnostic, idempotente, `--dry-run`, `--client`): copia plantilla `.brain/` → rellena config (03-projects) → instala git+agent hooks → graphify inicial → `register-to-dev-brain` → `--client` fija deny y no-federa → escribe bootstrap parametrizado → doctor. Arreglar drift `02-projects/`→`03-projects/` en plantilla + desplegados. Skill CKIS + `scripts/`. Doc: `docs/brain-init-pipeline.md`.
