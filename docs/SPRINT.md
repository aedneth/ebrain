...
- [ ] 4.6h1 **Write-back floor (BUILD FIRST)** — `~/eBrain/memory/` repo git = source `agent-memory` (markdown→git→`gbrain sync`, NO put directo); campo `agent:` en `log-session.sh`; Codex session logging (subagent_stop + git post-commit); sweep `sessions-federate` con filtros trust-policy (brisas/dekko nunca) + redact-scan. Cubierto por `ckis-backup-all`.
