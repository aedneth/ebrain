...
- [ ] 4.6 **Gobernanza del cerebro Codex** (ROUTING §2.1 — el driver primario $2500 hoy sin candados). (a) `~/.codex/config.toml`: sandbox + **paths de cliente denegados** (`brisas-del-golfo`, `dekko-floors`) + `.env*`/credenciales fuera de alcance; (b) **`AGENTS.md`** con los guidance blocks (search vía ebrain MCP, disciplina SOP, secretos, session-log+CHANGELOG); (c) **MCP ebrain registrado en codex-cli** (misma memoria + skills federadas); (d) **verificar expiry** de los créditos + burn-ledger semanal; (e) plan de reversión si expiran. Verificar `/codex` operativo despachando UNA tarea atómica auditada. Doc: `docs/tier0-playbook.md`. codex-cli 0.144.1 ✓.
