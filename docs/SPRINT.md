...
- [x] 3.9 **[NUEVO — Eduardo] Federación de skills** (diseñada, `docs/skill-federation.md`): ebrain como capa de skills unificada/opcional/agent-agnostic sobre gstack (54) + ckis-skills (35) + Company Brain (14) + workflows skillificados. Mecanismo = **skillpack nativo de gbrain** (`list_skills`/`get_skill` vía MCP, opcional, agent-agnostic). Impl (3.9a-d: registry.yaml + skills-publish + validación + patrón skillify) diferida tras F3; converge con 2.6b (adaptadores por host) en F4.
