...
- [ ] 4.6h3 **`remember` skill + CLI** (`harness/core/remember.sh` + `ebrain remember`) → `registry.yaml` (`source: skillified`) + línea en NORMS; refuse en repos deny.
- [ ] 4.6h4 **NORMS.md único + render de bloque gestionado** (`<!-- ebrain-norms:begin/end -->`) a CLAUDE.md / AGENTS.md; termina la divergencia canary-vs-RAM.
- [ ] 4.6h5 **Manifests + `ebrain harness install <agent>`** — claude+codex primero (CERO cambio de comportamiento = validación), luego gemini-cli como test de aceptación (5 criterios). `overlay/codex-harness/` migra a adapter `codex`.
