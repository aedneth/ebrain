...
- [x] 4.3 **`cli/route.ts` implementado + probado** (bun, ~230 líneas, build OK). Carga `routing.yaml` (Bun.YAML nativo), clasifica (keywords + `--cap`), llama array `models` (failover server-side), `provider:{data_collection:deny, max_price}` + `usage:{include:true}` (costo USD real), loguea `{ts,cap,model,tokens_in,tokens_out,usd}` a `spend.jsonl`, imprime `model=… cost=$…`, aborta si cap excedido. Doble candado frontier (regex hardcode + config). **Tests: 9/9 pasan** (`route.test.ts`: clasificación, cap excedido, frontier ban, spend por-mes) + integración hard-stop. Launcher `~/.config/ebrain/ebrain-route` (cwd neutro + sourcea key).
- [x] 4.4 **`~/.config/ebrain/routing.yaml` escrito** con el registry de 4.1. Fixes del audit Fable integrados: `monthly_usd:4` (≤ crédito $5), capacidad **`reasoning`** añadida (7ª — landing del benchmark 4.5), `max_price` en `request_defaults`, fallbacks de `long_context` por ventana (≥1M ctx). `frontier.auto_escalate:false` en config Y hardcodeado.
