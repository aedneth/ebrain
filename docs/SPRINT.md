...
- [~] 4.6 **Gobernanza del cerebro Codex** (ROUTING §2.1). Flujo real (Eduardo): `--sandbox danger-full-access` en dirs aislados → control = **harness (hooks) + normas + MCP + aislamiento**, NO approval-gating. **HECHO:** (0) **harness de hooks** (`overlay/codex-harness/`, install idempotente): `block-secret-read.sh` (pre_tool_use — candado técnico de secretos, deny+exit2, 3/3 casos probados) + `session-context.sh` (session_start — contexto ebrain); `codex doctor` verde; contrato de hooks de Codex reverse-engineered (Claude-compatible, `~/.codex/hooks/hooks.json`); (b) **`~/.codex/AGENTS.md`** (normas blandas); (c) **MCP ebrain en codex-cli** (enabled). **PENDIENTE:** trust vivo del hook (Eduardo, próxima sesión codex); (d) **expiry créditos + burn-ledger** [Eduardo]; (e) reversión; `/codex` op-check; `docs/tier0-playbook.md`. codex-cli 0.144.1 ✓.
...
### 4.C.2 Harness unificado provider-agnostic (**ADR-002 — 2.6b ejecutado; audit Fable 5**)
> Deja de re-implementar el harness CKIS por-agente. UNA capa canónica (`~/eBrain/harness/`) + adaptadores finos (manifest) + write-back loop (memoria agéntica permanente cross-agente). Hallazgo duro: **Codex, driver primario, deja CERO session logs hoy** (los 348 del Dev Brain son de Claude) → amnésico en escritura.
- [x] 4.6h1 **Write-back floor — HECHO + probado e2e.** `~/eBrain/memory/` repo git = source `agent-memory` (federated, 39 páginas embebidas); `harness/core/log-session.sh` agent-agnostic (deriva repo del `cwd`, estampa `agent:`, sin dependencia de transcript); Codex cableado (`subagent_stop` → `~/.codex/hooks/log-session.sh` wrapper AGENT_NAME=codex); `scripts/sessions-federate` (sweep con trust-policy slug+remote + redact-scan). **Primer sweep: 38 sesiones federadas, 0 clientes colados (brisas/dekko excluidos), acceptance query responde (score 0.85).** En `ckis-backup-all` (target agent-memory→aedneth/agent-memory). `memory/` gitignored del repo ebrain.
- [x] 4.6h2 **Guard merge + contract freeze** — `harness/core/guard-secrets.sh` (dual-output JSON+exit2) es el canon; `~/.claude/hooks/block-env-read.sh` + `~/.codex/hooks/block-secret-read.sh` ahora son **wrappers** (configs intactas, cero riesgo). `harness/contract/events.md` (contrato congelado) + 10 fixtures + `contract-test.sh`. **Paridad probada: 10/10 ok, 0 divergencias vs el guard viejo** → comportamiento idéntico. (Wire a `ebrain doctor` cuando exista, 5.2.)
- [x] 4.6h3 **`remember` skill + CLI — HECHO + probado.** `harness/core/remember.sh` (página tipada `agent-learning` → `agent-memory` + `gbrain sync`; **fail-closed**: niega repos de cliente por slug+remote y niega texto con secreto obvio); `harness/skills/remember/SKILL.md` federada al skillpack (`source: harness`, first-priority) → MCP `list_skills` la expone a todo agente (76 skills). CLI unificado `cli/ebrain` (remember/q/route/harness/norms/federate) instalado en `~/.local/bin/ebrain`. **Verificado: 3 negativas de seguridad (cliente/secreto/vacío) + learning real recuperado a 0.847.** Commit `e2170d3`.
- [x] 4.6h4 **NORMS.md único + render — HECHO + probado.** `harness/core/NORMS.md` = fuente única cross-agente; `harness/core/render-norms.sh` inyecta bloque gestionado `<!-- ebrain-norms:begin/end -->` (idempotente, **0-diff al re-render**, backup previo, preserva todo fuera del bloque). Renderizado en `~/.codex/AGENTS.md` (dedup de su prosa → header + bloque) y `~/.claude/CLAUDE.md` (append; canary + refuerzo intactos). Termina la divergencia canary-vs-RAM. Commit `13cf2ed`.
...
- [x] 4.8 **HECHO** — LLM de chat de gbrain → OpenRouter. `models.default=openrouter:minimax/minimax-m3` (el knob que usa `think`; su default de fábrica era `anthropic:claude-opus-4-7`) + `GBRAIN_CHAT_MODEL` en ambos launchers (repo+live sincronizados). **`think` probado 100% en OpenRouter: `Pages:40 | Citations:8`, sin ANTHROPIC_API_KEY.** Embeddings intactos (OpenAI 3-large @1536d — OpenRouter no tiene embeddings API; regresión query OK 0.85). **Gap: spend de gbrain NO entra en el ledger local** (solo route.ts); cap real = OpenRouter server-side (instrumentar `src:gbrain` = mejora del motor ebrain). Nota F5: `models.tier.subagent` necesita modelo tool-capable. Detalle: `docs/runbook.md`.
- [x] 4.9 **HECHO — Evaluación Hermes = DEFER** (`docs/hermes-evaluation.md`). Desviación consciente del SPRINT: la eval concluye **NO instalar** (un "no" fundamentado ES resultado válido). 4 razones duras (RAM 4GB, valor del routing ya capturado en route.ts, MCP stdio-local → Hermes remoto amnésico, amplifica el canal de exfil recién cerrado), confirmadas por Fable. Costos (VPS $5 vs Modal/Daytona serverless) + 3 condiciones de revisita + guardrails si se adopta. **Gate humano: Eduardo decide; la eval recomienda esperar.**
- [x] 4.10 **GATE F4 `[AUDIT_PASS]`** — criterios: 3+ tareas ruteadas con costo logueado (✓ 6 en spend.jsonl, ganadores correctos), registry verificado (✓ `model-registry.md` live 2026-07-11), benchmark resuelto (✓ `benchmark-routing-cost.md` ~31× + `benchmark-qmd-vs-ebrain.md`), playbook Tier 0 escrito (✓ `tier0-playbook.md`), evaluación Hermes entregada (✓ DEFER). Extras F4: harness unificado H1–H5 + arsenal 6 agentes + cross-provider probado + gbrain chat→OpenRouter (`think` live). **F4 CERRADA.**
...
- [x] 5.4 **HECHO** — `.brain` de ebrain conectado a Dev Brain vía `brain-init` (trust=own): skeleton `.brain` + 5 git hooks + registro en `projects.json` + Claude hooks (merge no-destructivo) + guidance block en CLAUDE.md. **Probado con un commit** (`75244e6`: pre-commit.security + post-commit dispararon limpio). Grafo inicial: `graphify update .` **sin LLM** → **cveió contaminado por vendor/ (12022 nodos)** → creado `.graphifyignore` (excluye vendor/memory/.brain/graphify-out) + rebuild limpio = **15 nodos, 0 refs a vendor** (bajo porque ebrain es mayormente bash; graphify parsea a fondo TS→route.ts). Sync a Dev Brain: **19 notas en `code-graph/ebrain/`** (appendSpend/applyFloor/capExceeded/classify/… + communities). **Blindaje:** `.gitignore` de ebrain ahora ignora `.brain/`/`.claude/`/`graphify-out/` (el auto-backup nunca los commiteará). **Limitación conocida:** el auto-rebuild por-commit (`post-commit.graphify`) no está cableado en graphify 0.6.7 (no auto-instala ese sub-hook); refrescar con `graphify update . && cp graphify-out/graph.json .brain/graph/` (o `/graphify .`); `sync-obsidian-graph.sh` corre por cadencia (cada 10 commits).
