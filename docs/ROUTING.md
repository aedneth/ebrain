...
- **Hooks (tripwire técnico, migrado a `harness/adapters/codex/` + `harness/core/`) [IMPLEMENTADO 2026-07-11]** — bajo full-access el `AGENTS.md` es blando; los hooks son un **tripwire que sube el costo de accidente** (NO un control duro: un `cp` en dos pasos o un intérprete no listado lo evaden — no es cerrable a nivel regex; el boundary real es aislamiento por directorio). Codex los soporta Claude-compatible (`~/.codex/hooks/hooks.json`). Wrappers del guard canónico `harness/core/guard-secrets.sh` (`pre_tool_use`: bloquea leer archivos de secretos/`printenv`/`export -p` al contexto — deny+exit2, 16 contract-tests) + `inject-context.sh` (`session_start`: contexto ebrain/normas). `codex doctor` verde.
